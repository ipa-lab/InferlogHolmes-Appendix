...
with pm.Model() as eight_schools:
    ...
    theta_raw = pm.Normal("theta_raw", mu=mu, sigma=1, shape=(8,))
...
with model:
    step = pm.HamiltonianMC(step_scale=0.25, target_accept=0.65)
    idata = pmd.debug(6000, tune=6000, step=step, chains=4, random_seed=rng)
