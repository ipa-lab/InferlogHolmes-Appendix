...
rng = np.random.default_rng(RANDOM_SEED)
with model:
    step = pm.HamiltonianMC(step_scale=0.2, adapt_step_size=True,target_accept=0.8)
    idata = pmd.debug(10000, tune=500, step=step, chains=4, random_seed=rng)

idata.to_netcdf("/app/study/results/Task_rotello1.nc")

# %%
idata=az.from_netcdf("/app/study/results/Task_rotello1.nc")
# %%
az.summary(idata)
#%%
az.plot_trace(idata)
