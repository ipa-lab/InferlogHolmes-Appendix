...
with pm.Model() as heit_rotello:
    ...
    dval = pm.Deterministic("dval", 0.5 * mud + 0.5 * 1/lambdad**0.5 * dval_raw)

    cval_raw = pm.Normal("cval_raw", mu=0, sigma=1, shape=(k,))
    cval = pm.Deterministic("cval", muc + 1/lambdac**0.5 * cval_raw)

    thetah = pm.Deterministic("thetah", Phi(dval - cval))
    thetaf = pm.Deterministic("thetaf", Phi(-dval - cval))
...
with model:
    ...
    #step = None
    idata = pmd.debug(1500, tune=500, step=step, chains=4, random_seed=rng)
    #idata = pmd.debug(800, tune=200, step=step, chains=4, random_seed=rng)

#idata.to_netcdf("/app/study/results/Task_rotello_1.nc")
raise SystemExit
# %%
with model:
    idata_prior = pm.sample_prior_predictive(draws=50, random_seed=rng)
# %%
az.plot_trace(idata_prior, var_names=["thetah", "thetaf"])
# %%
import matplotlib.pyplot as plt
plt.hist(idata_prior.prior['thetah'].values.flatten())
...
n1
# %%
idata = az.from_netcdf("/app/study/results/Task_rotello_1.nc")
