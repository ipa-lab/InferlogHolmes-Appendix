...
with model:
    step = pm.HamiltonianMC(step_scale=0.25, target_accept=0.8)
    idata = pmd.debug(3000, tune=500, step=step, chains=4, random_seed=rng)

idata.to_netcdf("/app/study/results/Task_schools2.nc")

# %%
idata = az.from_netcdf("/app/study/results/Task_schools2.nc")

az.plot_trace(idata, var_names=["tau"], combined=True)
# %%
