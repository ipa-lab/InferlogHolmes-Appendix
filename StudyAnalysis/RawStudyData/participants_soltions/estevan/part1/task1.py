...
with pm.Model() as heit_rotello:
    mud = pm.Normal("mud", mu=0., sigma=5.) # I think sigma should be larger
    muc = pm.Normal("muc", mu=0., sigma=5.)

    lambdad = pm.Gamma("lambdad", alpha=1., beta=0.5) # I think beta has to be rare
    lambdac = pm.Gamma("lambdac", alpha=1., beta=0.5)
    
    dval_raw = pm.Normal("dval_raw", mu=0, sigma=5, shape=(k,)) # sigma should probably be larger?
    dval = pm.Deterministic("dval", muc + 1/lambdac**0.5 * dval_raw)

    cval_raw = pm.Normal("cval_raw", mu=0, sigma=5, shape=(k,))
...
#%%
idata = az.from_netcdf("/app/study/results/Task_rotello.nc")
# %%
