...
class CFGBuilder():
    ...
    def get_cfg(self, node: ast.AST, breaknode:Optional[CFGNode], continuenode:Optional[CFGNode]) -> CFG:
        ...
        if isinstance(node, Block):
            ...
            print(f"Warning: Unsupported node {ast.dump(node)} {ast.unparse(node)}")
            cfgnode = SkipNode(node_id, None)
            nodes.add(cfgnode)
            add_edge(startnode, cfgnode)
            add_edge(cfgnode, endnode)
            # Dont error -> skip node
            # This may break some top-level dependencies

            # raise Exception(f"Unsupported node {ast.dump(node)} {ast.unparse(node)}")
