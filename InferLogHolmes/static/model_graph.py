...
def get_graph(program: lasapp.ProbabilisticProgram, model: lasapp.Model):
...
def merge_nodes_by_name(model_graph, label_method="name"):
    if label_method not in ("name", "source"):
        raise Exception(f"Unknown label method {label_method}")
    
    name_to_rvs = {}
    for i, rv in model_graph.random_variables.items():
        name = rv.name if label_method == "name" else rv.address_node.source_text
        if name not in name_to_rvs:
            name_to_rvs[name] = []
        name_to_rvs[name].append(rv)
...
def  plot_model_graph(model_graph, filename="model.gv", format="pdf", view=True, label_method="name"):
